...
class App extends Component {
  state = {activeId: tabsList[0].tabId, score: 0, count: 0, over: true}
  ...
  onSelect = id => {
    if (id === this.imgid) {
      ...
      this.imgUrl = imagesList[this.M].imageUrl

      this.imgid = imagesList[this.M].id
    } else {
  ...
  render() {
    const {activeId, score, count, over} = this.state
    const seconds = 60 - count
    if (seconds === 0) {
      this.clearTimer()
    }
      ...
      return (
        <div className="bg-container">
          ...
          <div className="mid-container">
            <img src={this.imgUrl} alt="match" className="mid-image" />
          </div>
    ...
    return (
      <div className="bg-container">
        ...
        <div className="results-card">
          ...
          <p className="score-card-title">{score}</p>
          <button className="playAgain" type="button" onClick={this.reset}>
            <img
